...
export const AuditLogView: React.FC<AuditLogViewProps> = ({
  ...
  const { auditEntries } = useFailureState();
  const [activeAuditFilter, setActiveAuditFilter] = useState<'all' | 'failures' | 'autonomy' | 'grounding' | 'conflicts' | 'trades'>('all');

  // Selected Intervention ID for details in Column 3
  const [selectedInterventionId, setSelectedInterventionId] = useState<string | null>(null);

  // Filtered failure audit entries
  const filteredFailureLogs = React.useMemo(() => {
    return auditEntries.filter(entry => {
      if (activeAuditFilter === 'all') return true;
      if (activeAuditFilter === 'failures') return entry.type === 'DEGRADATION' || entry.type === 'QUARANTINE' || entry.type === 'BREACH';
      if (activeAuditFilter === 'autonomy') return entry.type === 'DEGRADATION' || entry.type === 'AUTONOMY_RESTORED' || entry.type === 'AUTO_DEFAULT_EXECUTED';
      if (activeAuditFilter === 'grounding') return entry.type === 'REGROUNDED' || entry.type === 'QUARANTINE';
      if (activeAuditFilter === 'conflicts') return entry.type === 'CONFLICT';
      if (activeAuditFilter === 'trades') return entry.type === 'AUTO_DEFAULT_EXECUTED';
      return true;
    });
  }, [auditEntries, activeAuditFilter]);
      ...
      <div className="flex-1 min-h-0 border border-border rounded-2xl bg-white shadow-xs overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-0 relative">
        ...
        <div className="lg:col-span-4 p-3 sm:p-3.5 flex flex-col justify-between bg-slate-50/40 overflow-y-auto min-h-0">
          <div className="space-y-2.5">
            ...
            {/* FAILURE, GROUNDING & AUTONOMY AUDIT LEDGER */}
            <div className="rounded-xl border border-amber-300 bg-white p-3 shadow-2xs space-y-2.5 font-mono">
              <div className="flex items-center justify-between pb-1.5 border-b border-border">
                <div className="flex items-center gap-1.5 text-xs font-bold text-slate-900">
                  <Activity className="w-3.5 h-3.5 text-amber-600" />
                  <span>Autonomy &amp; Failure Trace Ledger</span>
                </div>
                <Badge variant="outline" className="text-[9px] bg-amber-50 text-amber-900 border-amber-300">
                  {filteredFailureLogs.length} Events
                </Badge>
              </div>
              ...
              <div className="flex items-center gap-1 overflow-x-auto pb-1 text-[9.5px]">
                {[
                  { id: 'all', label: 'All' },
                  { id: 'failures', label: 'Failures & Step-downs' },
                  { id: 'autonomy', label: 'Autonomy Changes' },
                  ...
                  <button
                    key={pill.id}
                    type="button"
                    onClick={() => setActiveAuditFilter(pill.id as any)}
                    className={`px-2 py-0.5 rounded-md whitespace-nowrap transition-all border ${
                      activeAuditFilter === pill.id
              ...
              {/* Scrollable List of Filtered Failure Entries */}
              <div className="space-y-1.5 max-h-[220px] overflow-y-auto pr-0.5 text-[10.5px]">
                {filteredFailureLogs.map((entry) => (
                  <div
                    key={entry.id}
                    className={`p-2 rounded-lg border leading-tight space-y-1 ${
                      entry.type === 'BREACH'
                        ? 'bg-red-50/70 border-red-200 text-red-950'
                        : entry.type === 'QUARANTINE' || entry.type === 'CONFLICT' || entry.type === 'DEGRADATION'
                        ? 'bg-amber-50/70 border-amber-200 text-amber-950'
                        : 'bg-slate-50 border-slate-200 text-slate-900'
                    }`}
                  >
                    <div className="flex items-center justify-between text-[9px] text-muted-foreground">
                      <span className="font-bold text-slate-800 uppercase">{entry.type.replace(/_/g, ' ')}</span>
                      <span>{new Date(entry.timestamp).toLocaleTimeString()}</span>
                    </div>
                    <div className="font-semibold text-slate-900 text-[10px]">
                      {entry.nodeName} ({entry.escalatedTo || entry.modelName})
                    </div>
                    <p className="text-[10px] text-slate-700 opacity-90">
                      {entry.trigger} {entry.resolution ? `· ${entry.resolution}` : ''}
                    </p>
                  </div>
                ))}
              </div>
